...
import json
import copy
from helpers.storage import read_schema, write_schema, is_github_mode
            ...
            if raw_value.strip() == "" or raw_value == "Any":
                ...
                if param_data.get("type") == "number":
                    ...
                    if raw_value.lower() == "true":
                        ...
                        final_value = None  # Should not happen if "Any" caught above
                else:
                ...
                if isinstance(n_raw_val, str):
                    if n_raw_val.strip() == "" or n_raw_val == "Any":
                        ...
                        if np.get("type") == "number":
                            ...
                            if n_raw_val.lower() == "true":
                                n_final_val = True
                            elif n_raw_val.lower() == "false":
                                n_final_val = False
                        else:
                            n_final_val = n_raw_val
                else:
...
def rebuild_schema_dry_run(schema_name: str, param_name: str, new_param_data: dict):
    ...
    try:
        ...
        original_export_schema = read_schema(schema_name)
...
def apply_updates(schema_map: dict):
    ...
    for schema_name, new_data in schema_map.items():
        try:
            success, msg = write_schema(schema_name, new_data, commit_message=f"Update {schema_name}")
            if success:
                success_count += 1
            else:
                errors.append(f"{schema_name}: {msg}")
        except Exception as e:
...
def render_diff_ui(original: dict, new: dict, param_focus: str):
    ...
    source_label = "Current (GitHub)" if is_github_mode() else "Current (GCS)"
...
def check_schema_health(schema_data: dict, repo_data: dict) -> dict:
    ...
    for param_name, schema_param in schema_data.items():
        ...
        if s_val in ("Any", "", None):
            s_val = None
        if e_val in ("Any", "", None):
            e_val = None
...
def update_schema_full(schema_name: str, repo_data: dict):
    ...
    """
    try:
        current_schema = read_schema(schema_name)
        ...
        if updates_made:
            success, msg = write_schema(schema_name, new_schema, commit_message=f"Sync {schema_name} with repo")
            if success:
                return True, []
            else:
                return False, [msg]
